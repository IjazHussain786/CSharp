    ...
    public class Dijkstra
    {
        public static void DijkstraAlgorithm(Dictionary<Node, List<Connection>> graph, Node source)
        {
            ...
            while (queue.Count != 0)
            {
                ...
                {
                    var minDistance = currentNode.DijkstraDistance + neighbor.Distance;
        ...
        public static void Main()
        {
            ...
            {
                Console.Write("Distance from {0} to {1}: {2}; ", source.Id, nodes[i].Id, nodes[i].DijkstraDistance);
                Console.WriteLine("Shortest Path: {0}", BuildPath(graph, source, nodes[i]));
            }
        }

        private static string BuildPath(Dictionary<Node, List<Connection>> graph, Node source, Node destination)
        {
            List<int> shortestPath = new List<int>();
            Node currentNode = destination;
            if (currentNode.DijkstraDistance == double.PositiveInfinity || currentNode.Id == source.Id)
            {
                return string.Empty;
            }
